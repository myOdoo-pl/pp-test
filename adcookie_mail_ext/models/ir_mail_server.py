...
class IrMailServer(models.Model):
    ...
    def build_email(self, email_from, email_to, subject, body, email_cc=None, email_bcc=None, reply_to=False,
        ...
        """
        # ------------ MODIFIED LINES -------------
        # _logger.info(f'EMAIL FROM 1: {email_from}')
        # _logger.info(f'EMAIL FROM 1 TYPE: {type(email_from)}')

        split_string = email_from.split(" <")
        user_name = split_string[0]

        split_email_address = self._get_default_from_address().split("@")
        email_address = split_email_address[0] + "@" + split_email_address[1]

        email_from = user_name + " <" + email_address + ">"

        # _logger.info(f'EMAIL FROM 2: {email_from}')
        # ORG: email_from = email_from or self._get_default_from_address()
        # ------------ MODIFIED LINES -------------
